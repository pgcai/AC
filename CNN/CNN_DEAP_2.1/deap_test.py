            ...
            with tf.Session() as sess:
                ...
                if ckpt and ckpt.model_checkpoint_path:
                    ...
                    for i in range(test_num_examples):
                        ...
                        print("{} {} {} {}".format(pred_correct_V, pred_correct_A, pred_correct_D, pred_correct_L))
                        print(pred_labels[i])  # 测试用
                        print(y_[i])
                        for j in range(4):
                            # print(y_[i][j])  # 测试用
                            # print("-----------")
                            ...
                            # else:
                            #     pred_labels[i][j] = 0
                            if j == 0 and y_[i][j*2] == int(pred_labels[i][j*2] > pred_labels[i][j*2+1]):
                                pred_correct_V += 1
                            if j == 1 and y_[i][j*2] == int(pred_labels[i][j*2] > pred_labels[i][j*2+1]):
                                pred_correct_A += 1
                            if j == 2 and y_[i][j*2] == int(pred_labels[i][j*2] > pred_labels[i][j*2+1]):
                                pred_correct_D += 1
                            if j == 3 and y_[i][j*2] == int(pred_labels[i][j*2] > pred_labels[i][j*2+1]):
                                pred_correct_L += 1
                        print("{} {} {} {}".format(pred_correct_V, pred_correct_A, pred_correct_D, pred_correct_L))
                        print("-------------我是可爱的分割线-----------")  # 测试用
